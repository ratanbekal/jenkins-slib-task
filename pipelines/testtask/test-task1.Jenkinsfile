node {
     stage("Slib-Ex1-Test-1-Stage") {
        getAPIsecret apitoken:'v6PxYYLe8rhKftISrfFM42vTNTIxeUt97JKgT1y5',sectet:"github",url:"https://api.thecatapi.com/v1/votes",body:"{\"image_id\":\"asf\",\"sub_id\":\"my-user-1234\",\"value\":1}"  
     }   
     stage("Slib-Ex1-Test-2-Stage") {
        //getAPIsecret apitoken:'v6PxYYLe8rhKftISrfFM42vTNTIxeUt97JKgT1y5',secret:"niket",url:"https://ugbdxukak0.execute-api.us-east-1.amazonaws.com/default/fakeCredentials" 
     }
     stage("Slib-Ex1-Test-3-Stage") {
        getAPIsecret apitoken:'v6PxYYLe8rhKftISrfFM42vTNTIxeUt97JKgT1y5',name:"niket",url:"https://api.genderize.io" 
     }
     stage("Slib-Ex1-Test-4-Stage") {
        getAPIsecret apitoken:'v6PxYYLe8rhKftISrfFM42vTNTIxeUt97JKgT1y5',name:"niket",url:"https://api.agify.io" 
     }
}


